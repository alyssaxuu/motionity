...
function updatePanel(selection){
    if (!selection) {
        ...
        presets.forEach(function(preset){
            $("#preset").append("<option value='"+preset.id+"'>"+preset.name+"</option>");
        });
        ...
        if (canvas.getActiveObject().get("type") == "rect") {
        ...
        } else if (canvas.getActiveObject().get("type") == "path" || canvas.getActiveObject().get("type") == "circle") {
            ...
            o_letter_slider = new RangeSlider(selectme, {
                ...
                min: -200,
                max: 200,
                value: parseFloat((canvas.getActiveObject().get("charSpacing")/10).toFixed(2)),
                onmove: function(x) {
            ...
            o_line_slider = new RangeSlider(selectme, {
                ...
                min: 1,
                max: 500,
                value: parseFloat((canvas.getActiveObject().get("lineHeight")*100).toFixed(2)),
                onmove: function(x) {
        ...
        if (canvas.getActiveObject().get("type") == "group") {
					if (canvas.getActiveObject().get("assetType")  == "animatedText") {
							...
							fonts.forEach(function(font){
									$("#font-picker").append("<option value='"+font+"'>"+font+"</option>");
							});
							$("#font-picker").val(animatedtext.find(x => x.id == canvas.getActiveObject().id).props.fontFamily);
							$("#font-picker").niceSelect();
							$("#text-color input").val(convertToHex(animatedtext.find(x => x.id == canvas.getActiveObject().id).props.fill))
							$("#color-text-side").css("background-color", animatedtext.find(x => x.id == canvas.getActiveObject().id).props.fill);
							text_animation_list.forEach(function(preset){
								$("#preset-picker").append("<option value='"+preset.name+"'>"+preset.label+"</option>");
							});
							$("#preset-picker").val(animatedtext.find(x => x.id == canvas.getActiveObject().id).props.preset);
							...
							$("#easing-picker").val(animatedtext.find(x => x.id == canvas.getActiveObject().id).props.easing);
							$("#easing-picker").niceSelect();
							$("#durationinput").val(animatedtext.find(x => x.id == canvas.getActiveObject().id).props.duration/1000);
							$("#masks").niceSelect();
					}
					/*
...
function updatePanelValues() {
    if (canvas.getActiveObject()) {
        ...
        if (objects.find(x => x.id == object.get("id"))) {
            if ($("#masks option[value='"+objects.find(x => x.id == object.get("id")).mask+"']").length == 0) {
                $("#masks").val("none");
        ...
        updateStrokeValues();
        $("#object-x input").val(parseFloat((object.get("left")-artboard.get("left")-((object.get("width")*object.get("scaleX")))/2).toFixed(2)));
        $("#object-y input").val(parseFloat((object.get("top")-artboard.get("top")-((object.get("height")*object.get("scaleY")))/2).toFixed(2)));
        ...
        } else if (object.get("type") == "path" || object.get("type") == "circle" || object.get("type") == "textbox") {
            colormode = "fill";
            o_fill.setColor(object.get("fill"));
        }
        if (tempstore) {
...
function updateInputs(id) {
    ...
    if (!isNaN(parseFloat($("#object-color-fill-opacity input").val())) && id == "object-color-fill-opacity") {
        ...
        colormode = "fill";
        o_fill.setColor("rgba("+o_fill.getColor().toRGBA()[0]+","+o_fill.getColor().toRGBA()[1]+","+o_fill.getColor().toRGBA()[2]+","+($("#object-color-fill-opacity input").val()/100)+")");
    }
    if (!isNaN(parseFloat($("#object-color-stroke-opacity input").val())) && id == "object-color-stroke-opacity") {
        ...
        colormode = "stroke";
        o_fill.setColor("rgba("+o_fill.getColor().toRGBA()[0]+","+o_fill.getColor().toRGBA()[1]+","+o_fill.getColor().toRGBA()[2]+","+($("#object-color-stroke-opacity input").val()/100)+")");
    }
        if (!isNaN(parseFloat($("#object-color-shadow-opacity input").val())) && id == "object-color-shadow-opacity") {
        ...
        colormode = "shadow";
        o_fill.setColor("rgba("+o_fill.getColor().toRGBA()[0]+","+o_fill.getColor().toRGBA()[1]+","+o_fill.getColor().toRGBA()[2]+","+($("#object-color-shadow-opacity input").val()/100)+")");
    }
}
    ...
    if (canvas.getActiveObjects().length > 0) {
        if ($(this).find("input").val() || type) {
					...
					if ($(this).attr("id") == "animated-text-duration") {
						...
						animatedtext.find(x => x.id == object.id).setProp({duration:$(this).find("input").val()*1000}, canvas);
						save();
						return false;
					}
					if ($(this).attr("id") == "object-volume") {
						newKeyframe("volume", canvas.getActiveObject(), currenttime, (parseFloat($(this).find("input").val())/200), true);
						canvas.getActiveObject().set("volume", (parseFloat($(this).find("input").val())/200));
					}
            editingpanel = true;
            var selection = false;
            ...
            object.set({
                left: parseFloat($("#object-x input").val())+artboard.get("left")+((object.get("width")*object.get("scaleX"))/2),
                top: parseFloat($("#object-y input").val())+artboard.get("top")+((object.get("height")*object.get("scaleY"))/2),
        ...
        if ($(this).attr("id") == "canvas-w" || $(this).attr("id") == "canvas-h") {
            ...
            if (activepreset != "custom") {
                if (presets.find(x => x.id == activepreset).width != $("#canvas-w input").val() || presets.find(x => x.id == activepreset).height != $("#canvas-h input").val()) {
                    activepreset = "custom";
                    updatePanel();
                }
            }
        ...
        if (!isNaN(parseFloat($("#canvas-color-opacity input").val()))) {
            ...
            colormode = "back";
            o_fill.setColor("rgba("+o_fill.getColor().toRGBA()[0]+","+o_fill.getColor().toRGBA()[1]+","+o_fill.getColor().toRGBA()[2]+","+($("#canvas-color-opacity input").val()/100)+")");
        }
    }
}
function setTextAnimation() {
	var object = canvas.getActiveObject();
	animatedtext.find(x=>x.id == object.id).reset($(this).parent().find("input").val(), animatedtext.find(x=>x.id == object.id).props, canvas);
}
...
function populateGrid(type) {
    if (type == "shape-tool") {
        ...
        shape_grid_items.forEach(function(item){
            $("#shapes-row").append("<div class='grid-item'><img onload='onLoadImage(this)' draggable=false src='"+item+"'></div>");
        })
        emoji_items.forEach(function(item){
            $("#emojis-row").append("<div class='grid-emoji-item'><img  onload='onLoadImage(this)' draggable=false src='"+item+"'></div>");
        })
    } else if (type == "image-tool") {
				$("#images-grid").html("");
        image_categories.forEach(function(category){
					$("#categories").append("<div class='category' data-name='"+category.name+"'><img onload='onLoadImage(this)' src='"+category.image+"'>"+category.name+"</div>");
				});
        ...
        uploaded_images.slice().reverse().forEach(function(item){
					if (!item.hidden) {
						flag = true;
            $("#images-grid").append("<div class='image-grid-item' data-src='"+item.src+"' data-type='"+item.type+"' data-key='"+item.key+"'><img class='delete-media' draggable=false src='assets/more-options.svg'><img draggable=false onload='onLoadImage(this)' class='image-thing' src='"+item.thumb+"'</div>");
					}
				});
				$("#landing").remove();
				if (!flag) {
					$("#upload-tabs").after('<div id="landing" class="upload-landing"><div id="landing-text">Your uploaded images will show up here for easy access.</div></div>')
				}
        ...
        uploaded_videos.slice().reverse().forEach(function(item){
					if (!item.hidden) {
						flag = true;
            $("#images-grid").append("<div class='video-grid-item' data-src='"+item.src+"' data-type='"+item.type+"' data-key='"+item.key+"'><img class='delete-media' draggable=false src='assets/more-options.svg'><img draggable=false onload='onLoadImage(this)' class='image-thing' src='"+item.thumb+"'></div>");
					}
				});
				$("#landing").remove();
				if (!flag) {
					$("#upload-tabs").after('<div id="landing" class="upload-landing"><div id="landing-text">Your uploaded videos will show up here for easy access.</div></div>')
				}
			...
			audio_items.forEach(function(item){
				if (item.src == background_key) {
					flag = true;
					$("#audio-list").append("<div class='audio-item audio-item-active' data-src='"+item.src+"'><div class='audio-preview'><img src='assets/play-button.svg'></div><img class='audio-thumb' src='"+item.thumb+"'><div class='audio-info'><div class='audio-info-title'>"+item.name+"</div><a href='"+item.link+"' target='_blank' class='audio-info-desc'>"+item.desc+"</a><div class='audio-info-duration'>"+item.duration+"</div></div></div></div>")
				} else {
					$("#audio-list").append("<div class='audio-item' data-src='"+item.src+"'><div class='audio-preview'><img src='assets/play-button.svg'></div><img class='audio-thumb' src='"+item.thumb+"'><div class='audio-info'><div class='audio-info-title'>"+item.name+"</div><a href='"+item.link+"' target='_blank' class='audio-info-desc'>"+item.desc+"</a><div class='audio-info-duration'>"+item.duration+"</div></div></div></div>")
				}
			})
			...
			text_animation_list.forEach(function(text){
				$(".animated-text-grid").append("<div class='animated-text-item noselect' data-id='"+text.name+"'><img draggable='false' class='noselect' src='"+text.src+"'></div>")
			});
			...
			text_items.sansserif.forEach(function(text){
				...
				$("#shapes-cont").append("<div id='item-text' class='add-text noselect' data-font='"+text.fontname+"' style='font-family: "+text.fontname+", sans-serif'>"+text.name+"</div>");
			});
			...
			text_items.serif.forEach(function(text){
				...
				$("#shapes-cont").append("<div id='item-text' class='add-text noselect' data-font='"+text.fontname+"' style='font-family: "+text.fontname+"'>"+text.name+"</div>");
			});
...
function dragObject(e){
    ...
    drag.appendTo("body");
    drag.css({position: "absolute", zIndex: 9999999, left:$(this).offset().left, top:$(this).offset().top, width:canvas.getZoom()*drag.width(), pointerEvents: "none", opacity:0});
    var pageX = e.pageX;
    var pageY = e.pageY;
    ...
    function dragging(e){
        ...
        if (overCanvas && canvas.getActiveObject() && !replacing && canvas.getActiveObject().type == "image" && (drag.hasClass("image-grid-item") || drag.hasClass("video-grid-item"))) {
            if (e.ctrlKey) {
    ...
    function released(e){
        ...
        if (move && !replacing) {
            if (drag.hasClass("grid-item")) {
              newSVG(drag.find("img").attr("src"), xpos, ypos, drag.width(), false);
                ...
                if (drag.attr("id") == "heading-text") {
                    newTextbox(50, 700, "Add a heading", canvasx-artboard.get("left"), canvasy-artboard.get("top"), drag.width(), false, drag.attr("data-font"));
                } else if (drag.attr("id") == "subheading-text") {
                    newTextbox(22, 500, "Add a subheading", canvasx-artboard.get("left"), canvasy-artboard.get("top"), drag.width(), false, drag.attr("data-font"));
                } else if (drag.attr("id") == "body-text") {
                    newTextbox(18, 400, "Add body text", canvasx-artboard.get("left"), canvasy-artboard.get("top"), drag.width(), false, drag.attr("data-font"));
                } else {
										newTextbox(18, 400, "Your text", canvasx-artboard.get("left"), canvasy-artboard.get("top"), drag.width(), false, drag.attr("data-font"));
								}
            ...
            if (drag.hasClass("grid-item")) {
                newSVG(drag.find("img").attr("src"), artboard.get("left")+(artboard.get("width")/2), artboard.get("top")+(artboard.get("height")/2), 100, true);
							} else if (drag.hasClass("image-external-grid-item")) {
								savePixabayImage(drag.attr("data-src"), artboard.get("left")+(artboard.get("width")/2), artboard.get("top")+(artboard.get("height")/2), 150);
							} else if (drag.hasClass("video-external-grid-item")) {
								savePixabayVideo(drag.attr("data-src"), drag.find("img").attr("src"),  artboard.get("left")+(artboard.get("width")/2),  artboard.get("top")+(artboard.get("height")/2));
							} else if (drag.hasClass("image-grid-item")) {
								$("#load-image").addClass("loading-active");
                loadImage(drag.attr("data-src"), artboard.get("left")+(artboard.get("width")/2), artboard.get("top")+(artboard.get("height")/2), 150, true);
            } else if (drag.hasClass("grid-emoji-item")) {
							$("#load-image").addClass("loading-active");
                loadImage(drag.find("img").attr("src"), artboard.get("left")+(artboard.get("width")/2), artboard.get("top")+(artboard.get("height")/2), 50, true);
						} else if (drag.hasClass("add-text")) {
                if (drag.attr("id") == "heading-text") {
                    newTextbox(50, 700, "Add a heading", artboard.get("left")+(artboard.get("width")/2), artboard.get("top")+(artboard.get("height")/2), drag.width(), true, drag.attr("data-font"));
                } else if (drag.attr("id") == "subheading-text") {
                    newTextbox(22, 500, "Add a subheading", artboard.get("left")+(artboard.get("width")/2), artboard.get("top")+(artboard.get("height")/2), drag.width(), true, drag.attr("data-font"));
                } else if (drag.attr("id") == "body-text") {
                    newTextbox(18, 400, "Add body text", artboard.get("left")+(artboard.get("width")/2), artboard.get("top")+(artboard.get("height")/2), drag.width(), true, drag.attr("data-font"));
                } else {
									newTextbox(18, 400, "Your text", artboard.get("left")+(artboard.get("width")/2), artboard.get("top")+(artboard.get("height")/2), drag.width(), true, drag.attr("data-font"));
								}
            } else if (drag.hasClass("video-grid-item")) {
							$("#load-video").addClass("loading-active");
                loadVideo(drag.attr("data-src"), artboard.get("left")+(artboard.get("width")/2), artboard.get("top")+(artboard.get("height")/2), true);
            }
        }
        drag.remove();
    }
...
function deleteMedia(e) {
    ...
    if (window.confirm("Are you sure you want to permanently delete this asset? It will also remove any instances of it in the canvas.")) {
			deleteAsset(key);
		}
}
...
function saveLayerName() {
    ...
    objects.find(x => x.id == $(".name-active").parent().parent().attr("data-object")).label = $(".name-active").val();
    save();
...
function loadMoreMedia() {
	...
	if (value != "" && page != false) {
		...
		if ($("#image-tool").hasClass("tool-active")) {
			var URL = "https://pixabay.com/api/?key="+API_KEY+"&q="+encodeURIComponent(value)+"&page="+page;
			$.getJSON(URL, function(data){
				if (parseInt(data.totalHits) > 0) {
						$.each(data.hits, function(i, hit){
							$("#images-grid").append("<div class='image-grid-item image-external-grid-item' data-src='"+hit.webformatURL+"'><a class='credit' href='"+hit.pageURL+"' target='_blank'>"+hit.user+"</a><img draggable=false onload='onLoadImage(this)' src='"+hit.webformatURL+"'</div>");
						});
				} else {
						page = false;
				}
			...
			var URL = "https://pixabay.com/api/videos/?key="+API_KEY+"&q="+encodeURIComponent(value)+"&page="+page;
			$.getJSON(URL, function(data){
				if (parseInt(data.totalHits) > 0) {
					$.each(data.hits, function(i, hit){
						var video = hit.videos.medium.url;
						$("#images-grid").append("<div class='image-grid-item video-external-grid-item' data-src='"+video+"'><a class='credit' href='"+hit.pageURL+"' target='_blank'>"+hit.user+"</a><div id='time-video'>"+fancyTimeFormat(hit.duration)+"</div><img draggable=false onload='onLoadImage(this)' src='assets/transparent.png'</div>");
						createVideoThumbnail(video, 250, 0, true).then(function(data){
							$(".image-grid-item[data-src='"+video+"']").find("img").attr("src", data);
						});
					});
				} else {
						page = false;
...
function search() {
	...
	if ($("#image-tool").hasClass("tool-active")) {
		...
		if (value != "") {
			...
			$.getJSON(URL, function(data){
				if (parseInt(data.totalHits) > 0) {
						...
						$("#shapes-cont").html("<div id='no-results'>Sorry, we couldn't find any results for &#x22;"+encodeURIComponent(value)+"&#x22;. Please try a different query.</div>");
				}
			});
		} else {
		...
		var URL = "https://pixabay.com/api/videos/?key="+API_KEY+"&q="+encodeURIComponent(value);
		$("#images-grid").html("");
		if (value != "") {
			$("#landing").addClass("hide-landing");
			$("#pixabay").addClass("hide-pixabay");
			$.getJSON(URL, function(data){
				if (parseInt(data.totalHits) > 0) {
						$.each(data.hits, function(i, hit){
							...
							$("#images-grid").append("<div class='image-grid-item video-external-grid-item' data-src='"+video+"'><a class='credit' href='"+hit.pageURL+"' target='_blank'>"+hit.user+"</a><div id='time-video'>"+fancyTimeFormat(hit.duration)+"</div><img draggable=false onload='onLoadImage(this)' src='assets/transparent.png'</div>");
							//createVideoThumbnail(video, 250, 0, true).then(function(data){
								$(".image-grid-item[data-src='"+video+"']").find("img").attr("src", "https://i.vimeocdn.com/video/"+hit.picture_id+"_640x360.jpg");
							//});
						});
				} else {
					$("#shapes-cont").html("<div id='no-results'>Sorry, we couldn't find any results for &#x22;"+encodeURIComponent(value)+"&#x22;. Please try a different query.</div>");
		...
		if (value == "") {
			$("#shapes-cont").html('<p class="row-title">Shapes</p><div class="gallery-row" id="shapes-row"></div><p class="row-title">Emojis</p><div class="gallery-row" id="emojis-row"></div>');
			...
			combined.forEach(function(item){
				if (item.indexOf(value) > -1) {
					...
					if (item.indexOf("emoji") > -1) {
						$(".gallery-row").append("<div class='grid-emoji-item'><img draggable=false src='"+item+"'></div>");
					} else {
						$(".gallery-row").append("<div class='grid-item'><img draggable=false src='"+item+"'></div>");
					}
				}
			})
			if (!flag) {
		...
		if (value == "") {
			...
			fonts.forEach(function(font){
				if (font.toLowerCase().indexOf(value) > -1) {
					...
					$("#shapes-cont").append("<div id='item-text' class='add-text noselect' data-font='"+font+"' style='font-family: "+font+"'>"+font+"</div>");
				}
			})
			if (!flag) {
				$("#shapes-cont").html("<div id='no-results'>Sorry, we couldn't find any results for &#x22;"+encodeURIComponent(value)+"&#x22;. Please try a different query.</div>");
...
function clearFilters() {
	if (canvas.getActiveObject()) {
		...
		obj.filters = $.grep(obj.filters, function(i){
			return (i.type != "BlackWhite" && i.type != "Invert" && i.type != "Sepia" && i.type != "Kodachrome" && i.type != "Polaroid" && i.type != "Technicolor" && i.type != "Brownie" && i.type != "Vintage");
		})
		canvas.renderAll();
	}
}
	...
	if (canvas.getActiveObject()) {
		...
		if ($(".status-active").attr("id") == "status-on") {
			if (obj.filters.find(x => x.type == "RemoveColor")) {
				...
				obj.filters.push(new f.RemoveColor({
...
function showMore() {
	$("#more-over").css("top", $("#more-tool").offset().top+5-($("#more-over").height()/4));
...
function handleLottieUpload() {
    ...
    reader.onload = function(event) {
			newLottieAnimation(artboard.get("left")+(artboard.get("width")/2), artboard.get("top")+(artboard.get("height")/2), event.target.result)
		}
