...
function updateSelection(e) {
    if (e.target.type == "activeSelection") {
        ...
        canvas.getActiveObjects().forEach(function(object) {
            if ($(".layer").length > 0 && $(".layer[data-object='"+object.get("id")+"']").length > 0) {
        ...
        if ($(".layer").length > 0 && $(".layer[data-object='"+e.target.get("id")+"']").length > 0) {
            $(".layer-selected").removeClass("layer-selected");
            $(".layer[data-object='"+e.target.get("id")+"']").addClass("layer-selected");
            if (e.e != undefined) {
                document.getElementsByClassName("layer-selected")[0].scrollIntoView();
            }
        }
    ...
    for (var i = 0; i < tempgroup.length; i++) {
        if (tempgroup[i].inGroup) {
            ...
            props.forEach(function(prop){
                if (prop != "top" && prop != "scaleY" && prop != "width" && prop != "height" && prop != "shadow.offsetX" && prop != "shadow.offsetY" && prop != "shadow.opacity" && prop != "shadow.blur" && prop != "lineHeight") {
            ...
            keyarr.forEach(function(keyframe){
                if (keyframe.name != "top" && keyframe.name != "scaleY" && keyframe.name != "width" && keyframe.name != "height" && keyframe.name != "shadow.offsetX" && keyframe.name != "shadow.offsetY" && keyframe.name != "shadow.opacity" && keyframe.name != "shadow.blur" && keyframe.name != "lineHeight") {
...
function reGroup(id) {
    var group = [];
    var objects = [];
    groups.find(x => x.id == id).objects.forEach(function(object){
...
function updateRecordCanvas() {
			...
			if (obj.filters) {
				if (obj.filters.length > 0) {
					...
					obj.filters.forEach(function(filter){
						if (filter.type == "BlackWhite" || filter.type == "Invert" || filter.type == "Sepia" || filter.type == "Kodachrome" || filter.type == "Polaroid" || filter.type == "Technicolor" || filter.type == "Brownie" || filter.type == "Vintage") {
							object.filters.push({type:filter.type});
						} else if (filter.type == "Brightness") {
							object.filters.push({type:filter.type, value:filter.brightness});
						} else if (filter.type == "Contrast") {
							object.filters.push({type:filter.type, value:filter.contrast});
						} else if (filter.type == "Vibrance") {
							object.filters.push({type:filter.type, value:filter.vibrance});
						} else if (filter.type == "Saturation") {
							object.filters.push({type:filter.type, value:filter.saturation});
						} else if (filter.type == "HueRotation") {
							object.filters.push({type:filter.type, value:filter.rotation});
						} else if (filter.type == "Blur") {
							object.filters.push({type:filter.type, value:filter.blur});
						} else if (filter.type == "Noise") {
							object.filters.push({type:filter.type, value:filter.noise});
						} else if (filter.type == "RemoveColor") {
							object.filters.push({type:filter.type, distance:filter.distance, color:filter.color});
						}
					});
					obj.filters = [];
					obj.applyFilters();
    ...
    const canvassave = canvas.toJSON(["volume", "audioSrc", "defaultLeft", "defaultTop", "defaultScaleX", "defaultScaleY", "notnew", "starttime", "top", "left", "width", "height", "scaleX", "scaleY", "flipX", "flipY", "originX", "originY", "transformMatrix", "stroke", "strokeWidth", "strokeDashArray", "strokeLineCap", "strokeDashOffset", "strokeLineJoin", "strokeMiterLimit", "angle", "opacity", "fill", "globalCompositeOperation", "shadow", "clipTo", "visible", "backgroundColor", "skewX", "skewY", "fillRule", "paintFirst", "clipPath", "strokeUniform", "rx", "ry", 'selectable', 'hasControls', 'subTargetCheck', 'id', 'hoverCursor', 'defaultCursor', 'isEditing', 'source', 'assetType', 'duration', 'inGroup']);
...
function save() {
    ...
    canvas.clipPath = null;
    state = canvas.toJSON(["volume", "audioSrc", "top", "left", "width", "height", "scaleX", "scaleY", "flipX", "flipY", "originX", "originY", "transformMatrix", "stroke", "strokeWidth", "strokeDashArray", "strokeLineCap", "strokeDashOffset", "strokeLineJoin", "strokeMiterLimit", "angle", "opacity", "fill", "globalCompositeOperation", "shadow", "clipTo", "visible", "backgroundColor", "skewX", "skewY", "fillRule", "paintFirst", "clipPath", "strokeUniform", "rx", "ry", 'selectable', 'hasControls', 'subTargetCheck', 'id', 'hoverCursor', 'defaultCursor', 'isEditing', 'source', 'assetType', 'duration', 'inGroup', 'filters']);
		canvas.clipPath = artboard;
    statearr = {keyframes:JSON.parse(JSON.stringify(keyframes)), p_keyframes:JSON.parse(JSON.stringify(p_keyframes)), objects:JSON.parse(JSON.stringify(objects)), colormode:colormode, duration:duration, currenttime:currenttime};
    if (undo.length >= 1) {
														...
														if (objects.find(x => x.id == object.get("id")).filters) {
															...
															objects.find(x => x.id == object.get("id")).filters.forEach(function(filter){
																if (filter.type == "Sepia") {
																	...
																	object.filters.push(new f.RemoveColor({
...
function keyframeChanges(object, type, id, selection) {
    if (object.get("type") == "rect") {
        object.set({rx:parseFloat($("#object-corners input").val()),ry:parseFloat($("#object-corners input").val())})
    } else if (object.get("type") == "textbox") {
        object.set({charSpacing:parseFloat($("#text-h input").val())*10, lineHeight:parseFloat($("#text-v input").val()/100)});
        canvas.renderAll();
    }
    canvas.renderAll();
    if (type && type == "opacity") {
        newKeyframe("opacity", object, currenttime, object.get("opacity"), true);
    } else if (type && type == "opacity3") {
        newKeyframe("charSpacing", object, currenttime, object.get("charSpacing"), true);
        newKeyframe("lineHeight", object, currenttime, object.get("lineHeight"), true);
    } else {
        if (id == "object-x" || id == "object-y") {
            ...
            newKeyframe("scaleY", object, currenttime, object.get("scaleY"), true);
            newKeyframe("width", object, currenttime, object.get("width"), true);
            newKeyframe("height", object, currenttime, object.get("width"), true);
            if (selection) {
                newKeyframe("left", object, currenttime, object.get("left"), true);
                newKeyframe("top", object, currenttime, object.get("top"), true);
            }
        ...
        } else if (id == "object-shadow-x" || id == "object-shadow-y" || id == "object-blur" || id == "object-color-stroke-opacity") {
            newKeyframe("shadow.color", object, currenttime, object.shadow.color, true);
            newKeyframe("shadow.opacity", object, currenttime, object.shadow.opacity, true);
...
function checkAnyKeyframe(id, prop, inst) {
    ...
    if (object.get("type") != "textbox" && (prop == "charSpacing" || prop == "lineHeight")) {
        return false;
    }
    if (object.get("type") == "group" && (prop == "shadow.opacity" || prop == "shadow.color" || prop == "shadow.offsetX" || prop == "shadow.offsetY" || prop == "shadow.blur")) {
        return false;
    }
...
async function playVideos(time) {
    objects.forEach(async function(object){
        ...
        if (object.get("id").indexOf("Video") >= 0 && (p_keyframes.find(x => x.id == object.id).trimstart+p_keyframes.find(x => x.id == object.id).start) <= time && p_keyframes.find(x => x.id == object.id).end >= time) {
            ...
            if ($(object.getElement())[0].paused == true) {
                $(object.getElement())[0].currentTime = parseFloat((((time - p_keyframes.find(x => x.id == object.id).start) + p_keyframes.find(x => x.id == object.id).trimstart)/1000).toFixed(2));
            }
            if (!recording) {
                var animation = {
                    value: 0
				...
				var instance = anime({
						...
						update: async function(){
							if (start && play && !paused) {
								if (!flag && (p_keyframes.find(x => x.id == object.id).start) <= currenttime && p_keyframes.find(x => x.id == object.id).end >= currenttime) {
									if (obj.get("src")) {
										obj.get("src").currentTime = (p_keyframes.find(x => x.id == object.id).trimstart-p_keyframes.find(x => x.id == object.id).start+currenttime)/1000;
										...
										audio.currentTime = (p_keyframes.find(x => x.id == object.id).trimstart-p_keyframes.find(x => x.id == object.id).start+currenttime)/1000;
										audio.play();
										flag = true;
									}
								} else if ((p_keyframes.find(x => x.id == object.id).start) >= currenttime || p_keyframes.find(x => x.id == object.id).end <= currenttime) {
									if (obj.get("src")) {
										obj.get("src").pause();
									}
								}
	...
	//return new Promise(function(resolve){
        ...
        keyframes.forEach(function(keyframe, index){
            ...
            if (groups.find(x => x.id == keyframe.id)) {
                ...
                if (currenttime < (p_keyframes.find(x => x.id == keyframe.id).trimstart+p_keyframes.find(x => x.id == keyframe.id).start)) {
                    object.set("visible", false);
                    inst.renderAll();
        ...
        objects.forEach(function(object){
          if (object.id.indexOf("Group") == -1) {
               const object2 = canvas.getItemById(object.id);
                if (currenttime < (p_keyframes.find(x => x.id == object.id).trimstart+p_keyframes.find(x => x.id == object.id).start)) {
        ...
        keyframes.forEach(function(keyframe, index){
            ...
            if (keyframe.t >= time && currenttime >= (p_keyframes.find(x => x.id == keyframe.id).trimstart+p_keyframes.find(x => x.id == keyframe.id).start)) {
                ...
                var instance = anime({
                  ...
                  update: function() {
                    if (start && !paused) {
                        if (currenttime < (p_keyframes.find(x => x.id == keyframe.id).trimstart+p_keyframes.find(x => x.id == keyframe.id).start) || currenttime > p_keyframes.find(x => x.id == keyframe.id).end || currenttime > duration) {
                            object.set("visible", false);
                            inst.renderAll();
                        } else {
                            setValue(keyframe.name, object, animation.value, inst);
...
function renderKeyframe(object, prop, time) {
    ...
    if (prop == "shadow.color") {
        if ($("#"+object.get("id")).find(".shadowcolor").is(":visible")) {
            time = time-parseFloat(p_keyframes.find(x => x.id == object.get("id")).start)
        }
        $("#"+object.get("id")).find(".shadowcolor").prepend("<div class='keyframe' data-time='"+time+"' data-object='"+object.get("id")+"' data-property='"+prop+"'></div>");
        $("#"+object.get("id")).find(".shadowcolor").find("[data-time='"+time+"']").css({left:(time/timelinetime), background:color});
    } else {
        if ($("#"+object.get("id")).find("."+prop).is(":visible")) {
            time = time-parseFloat(p_keyframes.find(x => x.id == object.get("id")).start)
        }
        $("#"+object.get("id")).find("."+prop).prepend("<div class='keyframe' data-time='"+time+"' data-object='"+object.get("id")+"' data-property='"+prop+"'></div>");
        $("#"+object.get("id")).find("."+prop).find("[data-time='"+time+"']").css({left:(time/timelinetime), background:color});
    }
}
...
function newKeyframe(property, object, time, value, render) {
    ...
    if ($.inArray(property, objects.find(x => x.id == object.get("id")).animate) != -1) {
        ...
        if (keyarr2.length == 0) {
            if (property == "left") {
                objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == property).value = object.get(property)-artboard.get("left");
            } else if (property == "top") {
                objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == property).value = object.get(property)-artboard.get("top");
            } else {
        ...
        if (keyarr.length == 0) {
            if (property == "left") {
                keyframes.push({t:time, name:property, value:value-artboard.get("left"), id:object.get("id"), easing:"linear"});
            } else if (property == "top") {
                keyframes.push({t:time, name:property, value:value-artboard.get("top"), id:object.get("id"), easing:"linear"});
            } else {
							keyframes.push({t:time, name:property, value:value, id:object.get("id"), easing:"linear"});
            }
            if (render && property != "top" && property != "scaleY" && property != "width" && property != "height" && property != "stroke" && property != "shadow.opacity" && property != "shadow.offsetX" && property != "shadow.offsetY" && property != "shadow.blur" && property != "lineHeight") {
            ...
            if (property != "top" && property != "scaleY" && property != "width" && property != "height" && property != "stroke" && property != "shadow.opacity" && property != "shadow.offsetX" && property != "shadow.offsetY" && property != "shadow.blur" && property != "lineHeight") {
                updateKeyframe($("#"+object.get("id")).find(".keyframe[data-time='"+time+"'][data-property='"+property+"']"), true);
            }
        }
    } else {
        if (property == "left") {
...
function manualKeyframe() {
    ...
    if (prop == "position") {
        ...
        prop = "shadow.color"
        newKeyframe("shadow.opacity", object, currenttime, object.get("shadow.opacity"), true);
        newKeyframe("shadow.offsetX", object, currenttime, object.get("shadow.offsetX"), true);
        newKeyframe("shadow.offsetY", object, currenttime, object.get("shadow.offsetY"), true);
        newKeyframe("shadow.blur", object, currenttime, object.get("shadow.blur"), true);
    } else if (prop == "text") {
        prop = "charSpacing"
        newKeyframe("lineHeight", object, currenttime, object.get("lineHeight"), true);
    }
        ...
        objects.find(x => x.id == object.get("id")).animate = [];
				if (object.get("assetType") == "audio") {
					...
					props.forEach(function(prop){
						...
						} else if (prop == "shadow.opacity" || prop == "shadow.blur" || prop == "shadow.offsetX" || prop == "shadow.offsetY" || prop == "shadow.color") {
								if (object.get("type") != "group") {
										if (prop == "shadow.color") {
												newKeyframe(prop, object, 0, object.shadow.color, true);
										} else if (prop == "shadow.opacity") {
												newKeyframe(prop, object, 0, object.shadow.opacity, true);
										} else if (prop == "shadow.offsetX") {
...
function animateProp(prop, object) {
	...
	if (prop == "left") {
			...
			newKeyframe("left", object, currenttime, object.get("left"), true);
			newKeyframe("top", object, currenttime, object.get("top"), true);
			objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "left").value = object.get("left")-artboard.get("left");
			objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "top").value = object.get("top")-artboard.get("top");
	} else if (prop == "scaleX") {
			newKeyframe("scaleY", object, currenttime, object.get("scaleY"), true);
			newKeyframe("width", object, currenttime, object.get("width"), true);
			newKeyframe("height", object, currenttime, object.get("height"), true);
			...
			objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "height").value = object.get("height");
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "width").value = object.get("width");
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "scaleY").value = object.get("scaleY");
	} else if (prop == "strokeWidth") {
		newKeyframe("stroke", object, currenttime, object.get("stroke"), true);
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "stroke").value = object.get("stroke");
		...
		newKeyframe("shadow.offsetY", object, currenttime, object.shadow.offsetY, true);
		newKeyframe("shadow.blur", object, currenttime, object.shadow.blur, true);
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "shadow.color").value = object.get("shadow.color");
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "shadow.opacity").value = object.get("shadow.opacity");
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "shadow.offsetX").value = object.get("shadow.offsetX");
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "shadow.offsetY").value = object.get("shadow.offsetY");
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "shadow.blur").value = object.get("shadow.blur");
			objects.find(x => x.id == object.get("id")).animate.push("shadow.opacity");
		...
		newKeyframe("lineHeight", object, currenttime, object.get("lineHeight"), true);
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "lineHeight").value = object.get("lineHeight");
			objects.find(x => x.id == object.get("id")).animate.push("lineHeight");
	}
...
function freezeProp(prop, object) {
	objects.find(x => x.id == object.get("id")).animate = $.grep(objects.find(x => x.id == object.get("id")).animate, function(e){
		return (e != prop);
	});
	...
	if (prop == "left") {
		...
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "shadow.opacity").value = object.shadow.opacity;
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "shadow.offsetX").value = object.shadow.offsetX;
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "shadow.offsetY").value = object.shadow.offsetY;
		objects.find(x => x.id == object.get("id")).defaults.find(x => x.name == "shadow.blur").value = object.shadow.blur;
		keyframes = $.grep(keyframes, function(e){
	...
	$(".keyframe[data-object='"+object.get("id")+"'][data-property='"+prop+"']").remove();
}
...
function toggleAnimateProp(e) {
    ...
    if (prop == "position") {
		...
		if ($(this).parent().parent().parent().find(".freeze").hasClass("frozen")) {
			...
			var propmatch = [prop, "scaleY", "width", "height", "top", "stroke", "shadow.opacity", "shadow.offsetX", "shadow.offsetY", "shadow.blur", "lineHeight"];
			props.forEach(function(p){
				if ($.inArray(p, propmatch) == -1) {
					if (object.get("type") == "textbox" && p == "charSpacing" || p == "lineHeight") {
						freezeProp(p, object);
...
function renderLayer(object, animate = false) {
    ...
    if (object.get("type") == "textbox") {
        src = 'assets/text.svg';
    } else if (object.get("type") == "rect" || object.get("type") == "group" || object.get("type") == "circle" || object.get("type") == "path") {
        src = 'assets/star.svg';
    ...
    const width = (p_keyframes.find(x => x.id == object.get("id")).end-p_keyframes.find(x => x.id == object.get("id")).trimstart)/timelinetime
    $("#inner-timeline").prepend("<div class='object-props' id='"+object.get("id")+"' style='width:"+((p_keyframes.find(x => x.id == object.get("id")).end-p_keyframes.find(x => x.id == object.get("id")).start)/timelinetime)+"px'><div class='row main-row'><div class='row-el' style='background-color:"+color+"'><div class='trim-row' style='left:"+leftoffset+"px;width:"+width+"px'></div></div></div></div>");
		if (object.get("assetType") == "audio") {
			...
			$("#layer-inner-list").prepend("<div class='layer' data-object='"+object.get("id")+"'><div class='layer-name'><img class='droparrow' src='assets/drop-arrow.svg' ><img class='layer-icon' src="+src+"><input class='layer-custom-name' value='"+objects.find(x => x.id == object.get("id")).label+"' disabled></span><div class='layer-options'><img class='"+freeze+"' src='assets/"+freeze+".svg' title='Toggle animation'></div></div><div class='properties'></div></div>");
		} else {
			$("#layer-inner-list").prepend("<div class='layer' data-object='"+object.get("id")+"'><div class='layer-name'><img class='droparrow' src='assets/drop-arrow.svg' ><img class='layer-icon' src="+src+"><input class='layer-custom-name' value='"+objects.find(x => x.id == object.get("id")).label+"' disabled></span><div class='layer-options'><img class='lock "+classlock+"' src='assets/"+srclock+".svg' title='Lock layer'><img class='"+freeze+"' src='assets/"+freeze+".svg' title='Toggle animation'></div></div><div class='properties'></div></div>");
		}
    $(".layer[data-object='"+object.get("id")+"']").find(".properties").toggle();
    ...
    if (object.selectable == false) {
        $(".layer[data-object='"+object.get("id")+"']").attr("draggable", false);
    }
}
...
function renderProp(prop, object) {
    var classfreeze = "";
		srcfreeze = "freeze";
    if ($.inArray(prop, objects.find(x => x.id == object.get("id")).animate) != -1) {
        classfreeze = "frozen";
        srcfreeze = "frozen"
    }
		if (prop == "shadow.color") {
    ...
    $("#"+object.get("id")).append("<div class='row "+prop+" keyframe-row' data-object='"+object.get("id")+"'><div class='row-el'></div></div>");
    if (prop == "left") {
        $(".layer[data-object='"+object.get("id")+"']").find(".properties").append("<div class='property-name' data-property='position'><span class='property-keyframe' title='Create a new keyframe'></span>Position<img class='freeze-prop "+classfreeze+"' src='assets/"+srcfreeze+".svg' title='Toggle animation'></div>");
    } else if (prop == "scaleX") {
        $(".layer[data-object='"+object.get("id")+"']").find(".properties").append("<div class='property-name' data-property='scale'><span class='property-keyframe' title='Create a new keyframe'></span>Scale<img class='freeze-prop "+classfreeze+"' src='assets/"+srcfreeze+".svg' title='Toggle animation'></div>");
    } else if (prop == "strokeWidth") {
        $(".layer[data-object='"+object.get("id")+"']").find(".properties").append("<div class='property-name' data-property='stroke'><span class='property-keyframe' title='Create a new keyframe'></span>Stroke<img class='freeze-prop "+classfreeze+"' src='assets/"+srcfreeze+".svg' title='Toggle animation'></div>");
    } else if (prop == "shadowcolor") {
        $(".layer[data-object='"+object.get("id")+"']").find(".properties").append("<div class='property-name' data-property='shadow'><span class='property-keyframe' title='Create a new keyframe'></span>Shadow<img class='freeze-prop "+classfreeze+"' src='assets/"+srcfreeze+".svg' title='Toggle animation'></div>");
    } else if (prop == "charSpacing") {
        $(".layer[data-object='"+object.get("id")+"']").find(".properties").append("<div class='property-name' data-property='text'><span class='property-keyframe' title='Create a new keyframe'></span>Text<img class='freeze-prop "+classfreeze+"' src='assets/"+srcfreeze+".svg' title='Toggle animation'></div>");
    } else {
        $(".layer[data-object='"+object.get("id")+"']").find(".properties").append("<div class='property-name' data-property='"+prop+"'><span class='property-keyframe' title='Create a new keyframe'></span>"+prop+"<img class='freeze-prop "+classfreeze+"' src='assets/"+srcfreeze+".svg' title='Toggle animation'></div>");
    }
    $("#"+object.get("id")).find(".keyframe-row"+"."+prop).toggle();
}
...
function newLayer(object) {
    ...
    if ((object.get("assetType") && object.get("assetType") == "video") || object.get("type") == "lottie" || object.get("assetType") == "audio") {
        objects.push({object:object, id:object.get("id"), label:object.get("id"), color:color, defaults:[], locked:[], mask:"none", start:0, end:object.get("duration")});
				if (object.get("duration") < duration) {
					p_keyframes.push({start:currenttime, end:object.get("duration")+currenttime, trimstart:0, trimend:object.get("duration")+currenttime, object:object, id:object.get("id")});	
				} else {
					p_keyframes.push({start:currenttime, end:duration-currenttime, trimstart:0, trimend:duration-currenttime, object:object, id:object.get("id")});	
				}
		} else {
			objects.push({object:object, id:object.get("id"), label:object.get("id"), color:color, defaults:[], locked:[], mask:"none"});
			if (object.get("notnew")) {
				p_keyframes.push({start:object.get("starttime"), end:duration-object.get("starttime"), trimstart:0, trimend:duration-currenttime, object:object, id:object.get("id")});
			} else {
				p_keyframes.push({start:currenttime, end:duration-currenttime, trimstart:0, trimend:duration-currenttime, object:object, id:object.get("id")});
			}
		}
					...
					if (prop == "lineHeight" || prop == "charSpacing") {
							...
							if (object.get("type") != "group") {
									if (prop == "shadow.color") {
										...
										objects.find(x => x.id == object.id).defaults.push({name:prop, value:object.shadow.blur});
									} else if (prop == "shadow.offsetX") {
										objects.find(x => x.id == object.id).defaults.push({name:prop, value:object.shadow.offsetX});
									} else if (prop == "shadow.offsetY") {
										objects.find(x => x.id == object.id).defaults.push({name:prop, value:object.shadow.offsetY});
									} else if (prop == "shadow.opacity") {
										objects.find(x => x.id == object.id).defaults.push({name:prop, value:object.shadow.opacity});
									}
							}
					} else {
...
function newVideo(file, src, x, y, duration, center) {
    ...
    canvas.bringToFront(newvid);
		if (center) {
			newvid.set("left", artboard.get("left")+(artboard.get("width")/2));
			newvid.set("top", artboard.get("top")+(artboard.get("height")/2));
			canvas.renderAll();
		}
...
function crop(obj){
    ...
    if (left < 0 && top > 0) {
        ...
        obj.set({cropX:left/cropobj.get("scaleX"),cropY:top/cropobj.get("scaleY"),height:height,width:width}).setCoords();
        canvas.renderAll();
        obj.set({
            left: cleft+((obj.get("width")*obj.get("scaleX"))/2),
            top: ctop+((obj.get("height")*obj.get("scaleY"))/2)
...
function newImage(file, x, y, width, center) {
    ...
    canvas.setActiveObject(newimg);
		if (center) {
			newimg.set("left", artboard.get("left")+(artboard.get("width")/2));
			newimg.set("top", artboard.get("top")+(artboard.get("height")/2));
			canvas.renderAll();
		}
    ...
    if (files2) {
        Array.from(files2).forEach(file => {
            ...
            if (file.size/1024/1024 <= 10) {
                $("#upload-button").html("<img src='assets/upload.svg'> Uploading...");
...
function lineJoin() {
    if ($(".line-join-active").attr("id") == "miter") {
        $(".line-join-active").find("img").attr("src", "assets/miter.svg");
    } else if ($(".line-join-active").attr("id") == "bevel") {
        $(".line-join-active").find("img").attr("src", "assets/bevel.svg");
    } else if ($(".line-join-active").attr("id") == "round") {
        $(".line-join-active").find("img").attr("src", "assets/round.svg");
    } else if ($(".line-join-active").attr("id") == "small-dash") {
        $(".line-join-active").find("img").attr("src", "assets/dash2.svg");
    }
    ...
    if ($(this).attr("id") == "miter") {
        ...
        canvas.getActiveObject().set({strokeWidth: 0, strokeDashArray: false});
        canvas.renderAll();
        updatePanelValues();
...
function alignText() {
    ...
    if ($(".align-text-active").attr("id") == "align-text-left") {
        $(".align-text-active").find("img").attr("src", "assets/align-text-left.svg");
    } else if ($(".align-text-active").attr("id") == "align-text-center") {
        $(".align-text-active").find("img").attr("src", "assets/align-text-center.svg");
    } else if ($(".align-text-active").attr("id") == "align-text-right") {
        $(".align-text-active").find("img").attr("src", "assets/align-text-right.svg");
    } else {
        $(".align-text-active").find("img").attr("src", "assets/align-text-justify.svg");
    }
    $(".align-text-active").removeClass("align-text-active");
    $(this).addClass("align-text-active");
    if ($(this).attr("id") == "align-text-left") {
        textalign = "left";
        $(this).find("img").attr("src", "assets/align-text-left-active.svg");
    } else if ($(this).attr("id") == "align-text-center") {
        textalign = "center";
        $(this).find("img").attr("src", "assets/align-text-center-active.svg");
    } else if ($(this).attr("id") == "align-text-right") {
        textalign = "right";
        $(this).find("img").attr("src", "assets/align-text-right-active.svg");
    } else {
        textalign = "justify";
        $(this).find("img").attr("src", "assets/align-text-justify-active.svg");
    }
	...
	if (canvas.getActiveObject().get("assetType")) {
		WebFont.load({
			...
			active: () => {
				...
				animatedtext.find(x=>x.id == object.id).reset(animatedtext.find(x=>x.id == object.id).text, $.extend(animatedtext.find(x=>x.id == object.id).props,{fontFamily:font}), canvas);
				save();
			}
	});
	save();
...
function setDuration(length) {
    ...
    $("#total-time input").val(("0"+minutes).slice(-2)+":"+("0"+Math.floor(seconds)).slice(-2)+":"+("0"+Math.floor((seconds%1)*100)).slice(-2));
    $(".object-props").each(function(){
        $(this).css("width", (duration/timelinetime)-(p_keyframes.find(x => x.id == $(this).attr("id")).start/timelinetime)+"px");
        p_keyframes.find(x => x.id == $(this).attr("id")).end = duration;
        if (p_keyframes.find(x => x.id == $(this).attr("id")).trimend > p_keyframes.find(x => x.id == $(this).attr("id")).end) {
            p_keyframes.find(x => x.id == $(this).attr("id")).trimend = duration;
            $(this).find(".trim-row").css("width", (duration/timelinetime)-(p_keyframes.find(x => x.id == $(this).attr("id")).trimstart/timelinetime)+"px");
        }
    })
    setTimelineZoom(timelinetime);
    save();
...
function renderTimeMarkers() {
    ...
    $("#time-numbers").html("");
    $("#time-numbers").append("<div class='time-number' style='margin-left:"+offset_left+"px'>"+timenumber+"s<span></span></div>");
    timenumber++;
    while (timenumber*1000 <= duration) {
        $("#time-numbers").append("<div class='time-number' style='margin-left:"+renderoffset+"px'>"+timenumber+"s<span></span></div>");
...
function setTimelineZoom(time) {
    $(".object-props").each(function(){
        $(this).offset({left:(p_keyframes.find(x => x.id == $(this).attr("id")).start/time)+$("#inner-timeline").offset().left+offset_left});
        $(this).css({width: ($(this).width()*timelinetime)/time});
        $(this).find(".trim-row").css({left:(p_keyframes.find(x => x.id == $(this).attr("id")).trimstart/time)});
        $(this).find(".trim-row").css({width: ($(this).find(".trim-row").width()*timelinetime)/time});
    })
    timelinetime = time;
    $(".keyframe").each(function(){
        $(this).offset({left:(($(this).attr("data-time"))/timelinetime)+$("#inner-timeline").offset().left+offset_left})
    });
    $("#seekbar").offset({left:($("#inner-timeline").offset().left+(currenttime/timelinetime))+offset_left});
    $("#inner-timeline").css({width: (duration/timelinetime)+50});
    $("#inner-seekarea").css({minWidth: (duration/timelinetime)+50});
    renderTimeMarkers();
}
...
function removeKeyframe() {
    keyframes = $.grep(keyframes, function(e){
        return (e.t != selectedkeyframe.attr("data-time") || e.id != selectedkeyframe.attr("data-object") || e.name != selectedkeyframe.attr("data-property"));
    });
    if (selectedkeyframe.attr("data-property") == "left") {
        keyframes = $.grep(keyframes, function(e){
            return (e.t != selectedkeyframe.attr("data-time") || e.id != selectedkeyframe.attr("data-object") || e.name != "top");
        });
    } else if (selectedkeyframe.attr("data-property") == "scaleX") {
        keyframes = $.grep(keyframes, function(e){
            return (e.t != selectedkeyframe.attr("data-time") || e.id != selectedkeyframe.attr("data-object") || e.name != "scaleY");
        });
        keyframes = $.grep(keyframes, function(e){
            return (e.t != selectedkeyframe.attr("data-time") || e.id != selectedkeyframe.attr("data-object") || e.name != "width");
        });
        keyframes = $.grep(keyframes, function(e){
            return (e.t != selectedkeyframe.attr("data-time") || e.id != selectedkeyframe.attr("data-object") || e.name != "height");
        });
    } else if (selectedkeyframe.attr("data-property") == "strokeWidth") {
        keyframes = $.grep(keyframes, function(e){
            return (e.t != selectedkeyframe.attr("data-time") || e.id != selectedkeyframe.attr("data-object") || e.name != "stroke");
        });
    } else if (selectedkeyframe.attr("data-property") == "shadow.color") {
        keyframes = $.grep(keyframes, function(e){
            return (e.t != selectedkeyframe.attr("data-time") || e.id != selectedkeyframe.attr("data-object") || e.name != "shadow.blur");
        });
        keyframes = $.grep(keyframes, function(e){
            return (e.t != selectedkeyframe.attr("data-time") || e.id != selectedkeyframe.attr("data-object") || e.name != "shadow.offsetX");
        });
        keyframes = $.grep(keyframes, function(e){
            return (e.t != selectedkeyframe.attr("data-time") || e.id != selectedkeyframe.attr("data-object") || e.name != "shadow.offsetY");
        });
        keyframes = $.grep(keyframes, function(e){
            return (e.t != selectedkeyframe.attr("data-time") || e.id != selectedkeyframe.attr("data-object") || e.name != "shadow.opacity");
        });
    } else if (selectedkeyframe.attr("data-property") == "charSpacing") {
        keyframes = $.grep(keyframes, function(e){
            return (e.t != selectedkeyframe.attr("data-time") || e.id != selectedkeyframe.attr("data-object") || e.name != "lineHeight");
        });
    }
...
function copyKeyframes() {
    ...
    clipboard.forEach(function(keyframe){
        ...
        if (keyprop == "left") {
            ...
            const keyarr2 = $.grep(keyframes, function(e){
                return (e.t == keyframe.t && e.id == keyframe.id && e.name == "lineHeight")
            });
            newKeyframe("lineHeight", canvas.getItemByid(keyframe.id), newtime, keyarr2[0].value, true);
        } else if (keyprop == "shadow.color") {
            var keyarr2 = $.grep(keyframes, function(e){
                return (e.t == keyframe.t && e.id == keyframe.id && e.name == "shadow.opacity")
            });
            newKeyframe("shadow.opacity", canvas.getItemById(keyframe.id), newtime, keyarr2[0].value, true);
            var keyarr2 = $.grep(keyframes, function(e){
...
function updateKeyframe(drag, newval, offset) {
    ...
    if (keyprop == "left") {
        ...
        const keyarr2 = $.grep(keyframes, function(e){
            return (e.t == parseFloat(keytime) && e.id == drag.attr("data-object") && e.name == "lineHeight")
        });
        var keyval2 = keyarr2[0].value;
        if (newval) {
        ...
        var keyarr2 = $.grep(keyframes, function(e){
            return (e.t == parseFloat(keytime) && e.id == drag.attr("data-object") && e.name == "shadow.opacity")
        });
        var keyval2 = keyarr2[0].value;
        if (newval) {
        ...
        var keyarr2 = $.grep(keyframes, function(e){
            return (e.t == parseFloat(keytime) && e.id == drag.attr("data-object") && e.name == "shadow.offsetX")
        });
        var keyval2 = keyarr2[0].value;
        if (newval) {
        ...
        var keyarr2 = $.grep(keyframes, function(e){
            return (e.t == parseFloat(keytime) && e.id == drag.attr("data-object") && e.name == "shadow.offsetY")
        });
        var keyval2 = keyarr2[0].value;
        if (newval) {
        ...
        var keyarr2 = $.grep(keyframes, function(e){
            return (e.t == parseFloat(keytime) && e.id == drag.attr("data-object") && e.name == "shadow.blur")
        });
        var keyval2 = keyarr2[0].value;
        if (newval) {
...
function keyframeSnap(drag) {
    if (shiftkeys.length == 0) {
        if (drag.offset().left > $("#seekbar").offset().left-5 && drag.offset().left < $("#seekbar").offset().left+5) {
            drag.offset({left: $("#seekbar").offset().left});
            $("#line-snap").offset({left: $("#seekbar").offset().left, top:drag.parent().parent().offset().top});
            $("#line-snap").css({height:drag.parent().parent().height()});
            $("#line-snap").addClass("line-active");
        } else {
            drag.parent().parent().find(".keyframe").each(function(index){
               if (!drag.is($(this))) {
                   if (drag.offset().left > $(this).offset().left-5 && drag.offset().left < $(this).offset().left+5) {
                       drag.offset({left: $(this).offset().left});
                       $("#line-snap").offset({left: $(this).offset().left, top:drag.parent().parent().offset().top});
                       $("#line-snap").css({height:drag.parent().parent().height()});
                       $("#line-snap").addClass("line-active");
                       return false;
                   }
...
function dragKeyframe(e){
    ...
    if (e.shiftKey) {
        if (!$(this).hasClass("keyframe-selected")) {
            shiftkeys.push({keyframe:this, offset:$(this).offset().left});
    ...
    function draggingKeyframe(e){
        ...
        if (shiftkeys.length == 0) {
            if (left > $("#timearea").offset().left+offset_left) {
                ...
                drag.offset({left: $("#timearea").offset().left+offset_left});
            }
            keyframeSnap(drag);
        } else {
            shiftkeys.forEach(function(key){
                if (key.keyframe != inst) {
                    $(key.keyframe).offset({left: key.offset + (e.pageX - pageX)});
    ...
    function releasedKeyframe(e){
        $('body').off('mousemove', draggingKeyframe).off('mouseup', releasedKeyframe);
        $("#line-snap").removeClass("line-active");
        if (move) {
            if (shiftkeys.length == 0) {
                ...
                var time = parseFloat((drag.position().left*timelinetime).toFixed(1));
                if (time%16.666 != 0) {
                    drag.offset({left: ((Math.ceil(time/16.666)*16.666)/timelinetime)+drag.parent().offset().left});
                ...
                shiftkeys.forEach(function(key) {
                    ...
                    if (time%16.666 != 0) {
                        $(key.keyframe).offset({left: ((Math.ceil(time/16.666)*16.666)/timelinetime)+$(key.keyframe).parent().offset().left});
    ...
    $('body').on('mouseup', releasedKeyframe).on('mousemove', draggingKeyframe);
}
...
function renderTime() {
    ...
    $("#current-time input").val(("0"+minutes).slice(-2)+":"+("0"+Math.floor(seconds)).slice(-2)+":"+("0"+Math.floor((seconds%1)*100)).slice(-2));
}

// Update current time (and account for frame "slots")
function updateTime(drag, check){
    if ($("#timeline").scrollLeft() > offset_left) {
        currenttime = parseFloat((((drag.position().left+$("#timeline").scrollLeft()-offset_left)*timelinetime)).toFixed(1));
    } else {
        currenttime = parseFloat((((drag.position().left+$("#timeline").scrollLeft()-offset_left)*timelinetime)).toFixed(1));
    }
    // Check for 60FPS playback, 16ms "slots"
...
function dragSeekBar(e){
    ...
    function dragging(e){
        paused = true;
        var left = offset.left + (e.pageX - pageX);
        if (left > $("#timearea").offset().left+offset_left && (left-$("#timearea").offset().left) < (duration/timelinetime)+offset_left) {
            drag.offset({left: left});
        } else if (left < $("#timearea").offset().left+offset_left) {
            drag.offset({left: offset_left+$("#timearea").offset().left});
        }
        if ($("#timeline").scrollLeft() > offset_left) {
            currenttime = parseFloat((((drag.position().left+$("#timeline").scrollLeft()-offset_left)*timelinetime)).toFixed(1));
        } else {
    ...
    function dragging(e){
        if (trim == "no") {
            ...
            if ($(".keyframe-row[data-object='"+drag.attr("id")+"']").is(":hidden")) {
                opened = true;
								$(".layer[data-object='"+drag.attr("id")+"']").find(".properties").toggle();
								$(".layer[data-object='"+drag.attr("id")+"']").find(".properties").toggleClass("layeron");
            ...
            if (initwidth+(e.pageX-pageX) < (duration/timelinetime)) {
                ...
                drag2.css({
                    width: (duration/timelinetime)-drag.position().left-$("#timeline").scrollLeft()+offset_left
                });
            }
...
function resetHeight() {
	...
	if ($("#upload-tool").hasClass("tool-active")) {
		...
		$("#browser").css("height", "calc(100% - "+(top+97+150)+"px)");
	} else {
		$("#browser").css("top", "110px");
		$("#browser").css("height", "calc(100% - "+(top+97+100)+"px)");
	}
	$("#timearea").css("height", top);
	$("#layer-list").css("height", top);
	$("#toolbar").css("height", "calc(100% - "+(top+97)+"px)");
	$("#canvas-area").css("height", "calc(100% - "+(top+97)+"px)");
...
function syncScrollHoz(el1, el2) {
  ...
  function performScroll($scrolled, $toScroll) {
    if (forcedScroll) return (forcedScroll = false);
    var percent = ($scrolled.scrollLeft() /
        ($scrolled.outerWidth())) * 100;
...
function seekTo(e) {
    ...
    if ($("#seekarea").scrollLeft() > offset_left) {
        currenttime = parseFloat((((e.pageX+$("#seekarea").scrollLeft()-$("#timearea").offset().left-offset_left)*timelinetime)).toFixed(1));
    } else {
        currenttime = parseFloat((((e.pageX+$("#seekarea").scrollLeft()-$("#timearea").offset().left-offset_left)*timelinetime)).toFixed(1));
    }
    if (currenttime < 0) {
...
function followCursor(e) {
	$("#seek-hover").css({opacity:.3});
	if ($("#seekarea").scrollLeft() > offset_left) {
			hovertime = parseFloat((((e.pageX+$("#seekarea").scrollLeft()-$("#timearea").offset().left-offset_left)*timelinetime)).toFixed(1));
	} else {
			hovertime = parseFloat((((e.pageX+$("#seekarea").scrollLeft()-$("#timearea").offset().left-offset_left)*timelinetime)).toFixed(1));
	}
...
function orderLayers() {
    $(".layer").each(function(index){
        ...
        canvas.renderAll();
        objects.splice($(".layer").length-index-1, 0, objects.splice(objects.findIndex(x => x.id == object.get("id")), 1)[0]);
    })
    save();
}
