 ...
 function animateText(group, ms, play, props, cv, id) {
	...
	class AnimatedText {
		...
		render(cv) {
			this.id = renderText(this.text, this.props, this.props.left, this.props.top, cv, this.id, false, 0);
		...
		reset(text, newprops, cv) {
			...
			this.text = text;
			this.props = newprops;
			this.inst = renderText(text, this.props, left, top, cv, this.id, true, start);
