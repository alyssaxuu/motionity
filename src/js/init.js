...
var keyframes = [];
var p_keyframes = [];
var props = ["left", "top", "scaleX", "scaleY", "width", "height", "angle", "opacity", "fill", "strokeWidth", "stroke", "shadow.color", "shadow.opacity", "shadow.offsetX", "shadow.offsetY", "shadow.blur", "charSpacing", "lineHeight"];
var objects = [];
...
// Get list of fonts
$.ajax({
    url: "https://www.googleapis.com/webfonts/v1/webfonts?key="+GOOGLE_FONTS_API_KEY+"&sort=alpha",
    type: 'GET',
    ...
    this.lottieItem.addEventListener('enterFrame', (e) => {
      this.canvas.requestRenderAll()
		...
		this.lottieItem.addEventListener('DOMLoaded', () => {
			...
			this.lottieItem.duration = this.lottieItem.getDuration(false)*1000;
			this.canvas.requestRenderAll();
			canvas.renderAll();
...
// Color picker events
o_fill.on('init', instance => {
    o_fill.hide();
}).on('change', instance => {
    if (canvas.getActiveObject()) {
        const object = canvas.getActiveObject();
        if (colormode == "fill") {
            $("#object-color-fill input").val(o_fill.getColor().toHEXA().toString().substring(0, 7));
            $("#object-color-fill-opacity input").val(Math.round((o_fill.getColor().toRGBA()[3] * 100) * 100) / 100);
            $("#color-fill-side").css("background-color", o_fill.getColor().toHEXA().toString().substring(0, 7));
            ...
            $("#object-color-stroke input").val(o_fill.getColor().toHEXA().toString().substring(0, 7));
            $("#object-color-stroke-opacity input").val(Math.round((o_fill.getColor().toRGBA()[3] * 100) * 100) / 100);
            $("#color-stroke-side").css("background-color", o_fill.getColor().toHEXA().toString().substring(0, 7));
            ...
            $("#object-color-shadow input").val(o_fill.getColor().toHEXA().toString().substring(0, 7));
            $("#object-color-shadow-opacity input").val(Math.round((o_fill.getColor().toRGBA()[3] * 100) * 100) / 100);
            $("#color-shadow-side").css("background-color", o_fill.getColor().toHEXA().toString().substring(0, 7));
          ...
          $("#color-chroma-side").css("background-color", o_fill.getColor().toHEXA().toString().substring(0, 7));
					if (obj.filters.find(x => x.type == "RemoveColor")) {
						obj.filters.find(x => x.type == "RemoveColor").color = o_fill.getColor().toRGBA().toString();
					}
					updateChromaValues();
				} else if (colormode == "text") {
					var obj = canvas.getActiveObject();
					$("#text-color input").val(o_fill.getColor().toHEXA().toString().substring(0, 7));
          $("#color-text-side").css("background-color", o_fill.getColor().toHEXA().toString().substring(0, 7));
					animatedtext.find(x => x.id == obj.id).setProps({fill:o_fill.getColor().toRGBA().toString()}, canvas);
				}
        canvas.renderAll();
    } else {
        if (colormode == "back") {
            $("#canvas-color input").val(o_fill.getColor().toHEXA().toString().substring(0, 7));
            $("#canvas-color-opacity input").val(Math.round((o_fill.getColor().toRGBA()[3] * 100) * 100) / 100);
            $("#color-side").css("background-color", o_fill.getColor().toHEXA().toString().substring(0, 7));
            canvas.setBackgroundColor(o_fill.getColor().toRGBA().toString());
            canvas.renderAll();
        }
    }
}).on("show", instance => {
	$(".pcr-current-color").html("<img id='eyedropper' src='assets/eyedropper.svg'>")
});
...
selectbox.on('beforestart', evt => {
    if ($(evt.event.target).hasClass("keyframe") || $(evt.event.target).attr("id") == "seekbar" || $(evt.event.target).parent().hasClass("main-row") || $(evt.event.target).hasClass("main-row") || $(evt.event.target).hasClass("trim-row") || evt.event.which === 3) {
        return false;
    }
}).on('start', evt => {
    ...
    if (evt.store.selected.length == 0) {
        ...
        evt.store.selected.forEach(function(key){
            shiftkeys.push({keyframe:key, offset:$(key).offset().left});
