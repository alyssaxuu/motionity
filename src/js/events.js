$(document).ready(function(){
    ...
    canvas.on('mouse:wheel', function(opt) {
      ...
      zoom *= 0.999 ** delta;
      $("#zoom-level span").html(kFormatter((zoom*100).toFixed(0))+"%");
      if (zoom > 20) zoom = 20;
    ...
    canvas.on('mouse:move', function(e){
       overCanvas = true; 
       if (e.target && !canvas.getActiveObject() && draggingPanel && e.target.type == "image") { 
            wip = true;
            e.target.hasControls = false;
    ...
    $(document).keyup(function(e) {
        ...
        if (e.keyCode == 32 && !editinglayer && !editingproject && $(e.target)[0].tagName != "INPUT") {
            spacerelease = true;
            spaceDown = false;
        ...
        if ((e.keyCode == 46 || e.key == 'Delete' || e.code == 'Delete' || e.key == 'Backspace') && !focus && !editinglayer) {
            ...
            autoKeyframe(obj, {action:"drag"}, false);
				...
				if (e.keyCode === 221 && canvas.getActiveObjects() && e.metaKey) {
					if (canvas.getActiveObjects().length == 1) {
						var obj = canvas.getActiveObject();
						$(".layer[data-object='"+obj.id+"']").prev().insertAfter($(".layer[data-object='"+obj.id+"']"));
				...
				if (e.keyCode === 219 && canvas.getActiveObjects() && e.metaKey) {
					if (canvas.getActiveObjects().length == 1) {
						var obj = canvas.getActiveObject();
						$(".layer[data-object='"+obj.id+"']").next().insertBefore($(".layer[data-object='"+obj.id+"']"));
    ...
    window.addEventListener('copy', function(e){
        ...
        if (canvas.getActiveObject() && shiftkeys.length == 0 && !canvas.getActiveObject().isEditing) {
        ...
        // Copy selected keyframe(s)
            ...
            shiftkeys.forEach(function(keyframe){
                ...
                var keyarr = $.grep(keyframes, function(e){
                    return (e.t == drag.attr("data-time") && e.id == drag.attr("data-object") && e.name == drag.attr("data-property"))
                }); 
            ...
            for (var i = 0; i < imgs.length; i++) {
                ...
                if (imgObj.size/1024/1024 <= 10) {
                    createThumbnail(imgObj, 250).then(function(data){
											saveFile(dataURItoBlob(data), imgObj, imgObj["type"].split('/')[0], "temp", true, true);
                    })
                } else {
    ...
    $(document).on("mousedown", function(e){
        // De-select keyframes
        if (!$("#keyframe-properties").is(e.target) && $("#keyframe-properties").has(e.target).length === 0 && !$(".keyframe").is(e.target) && $(".keyframe").has(e.target).length === 0) {
        ...
        // Hide color picker
        if (!$(".object-color").is(e.target) && $(".object-color").has(e.target).length === 0 && !$(".pcr-app").is(e.target) && $(".prc-app").has(e.target).length === 0 && !$(".pcr-selection").is(e.target) && $(".pcr-selection").has(e.target).length === 0 && !$(".pcr-swatches").is(e.target) && $(".pcr-swatches").has(e.target).length === 0 && !$(".pcr-interaction").is(e.target) && $(".pcr-interaction").has(e.target).length === 0) {
            o_fill.hide();
        }
        
        // Hide zoom controls
        if (!$("#other-controls").is(e.target) && $("#other-controls").has(e.target).length === 0) {
            ...
            $("#zoom-level img").removeClass("zoom-open");
				...
				// Hide more menu
				if (!$("#more-tool").is(e.target) && $("#more-tool").has(e.target).length === 0 && !$("#more-over").is(e.target) && $("#more-over").has(e.target).length === 0) {
					hideMore();
				}
    });
    
    // Detect focus on an input in the properties
    $(document).on("focus", ".property-input", function(){
        focus = true; 
    ...
    sortable('#layer-inner-list', {customDragImage: (draggedElement, elementOffset, event) => {
                return {
    ...
    updateBrowser("shape-tool");
		...
		function initFilterSliders() {
			var filters = ['filter-brightness', 'filter-contrast', 'filter-saturation', 'filter-hue', 'filter-vibrance'];
