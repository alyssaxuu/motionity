...
function checkDB() {
	db.collection('projects').get().then(project => {
		if (project.length == 0) {
			...
			const inst = canvas.toDatalessJSON(["volume", "audioSrc", "defaultLeft", "defaultTop", "defaultScaleX", "defaultScaleY", "notnew", "starttime", "top", "left", "width", "height", "scaleX", "scaleY", "flipX", "flipY", "originX", "originY", "transformMatrix", "stroke", "strokeWidth", "strokeDashArray", "strokeLineCap", "strokeDashOffset", "strokeLineJoin", "strokeMiterLimit", "angle", "opacity", "fill", "globalCompositeOperation", "shadow", "clipTo", "visible", "backgroundColor", "skewX", "skewY", "fillRule", "paintFirst", "strokeUniform", "rx", "ry", 'selectable', 'hasControls', 'subTargetCheck', 'id', 'hoverCursor', 'defaultCursor', 'filesrc', 'isEditing', 'source', 'assetType', 'duration', 'inGroup', 'filters']);
...
function autoSave() {
	if (checkstatus) {
		...
		const inst = canvas.toDatalessJSON(["volume", "audioSrc", "defaultLeft", "defaultTop", "defaultScaleX", "defaultScaleY", "notnew", "starttime", "top", "left", "width", "height", "scaleX", "scaleY", "flipX", "flipY", "originX", "originY", "transformMatrix", "stroke", "strokeWidth", "strokeDashArray", "strokeLineCap", "strokeDashOffset", "strokeLineJoin", "strokeMiterLimit", "angle", "opacity", "fill", "globalCompositeOperation", "shadow", "clipTo", "visible", "backgroundColor", "skewX", "skewY", "fillRule", "paintFirst", "strokeUniform", "rx", "ry", 'selectable', 'hasControls', 'subTargetCheck', 'id', 'hoverCursor', 'defaultCursor', 'filesrc', 'isEditing', 'source', 'assetType', 'duration', 'inGroup']);
		canvas.clipPath = artboard;
	...
	db.collection("projects").doc({id: 1}).get().then(document => {
		...
		canvas.loadFromJSON(JSON.parse(project.canvas), function() {
				...
				objects.forEach(function(object) {
					...
					if (!canvas.getItemById(object.id).get("assetType") || canvas.getItemById(object.id).get("assetType") != "audio") {
...
async function saveFile(thumbnail, file, type, name, place, hidden) {
	...
	if (type === "image") {
			uploaded_images.push({src:file, thumb:thumbnail, key:key, type:"image", hidden:false});
			populateGrid("images-tab");
	} else if (type === "video") {
			uploaded_videos.push({src:file, thumb:thumbnail, key:key, type:"video", hidden:false});
			populateGrid("videos-tab");
	}
	$("#upload-button").html("<img src='assets/upload.svg'> Upload media");
	$("#upload-button").removeClass("uploading");
	if (place) {
		loadImage(file, artboard.get("left")+(artboard.get("width")/2), artboard.get("top")+(artboard.get("height")/2), 200);
	}
	save();
}
...
function deleteAsset(key) {
	db.collection("assets").doc({key:key}).get().then(asset => {
...
function getAssets() {
	db.collection("assets").get().then(assets => {
		...
		if (assets === undefined) {
			getAssets();
				...
				assets.forEach(function(asset){
						if (asset.type == "image") {
								uploaded_images.push({src:asset.src, thumb:asset.thumb, key:asset.key, type:"image", hidden:asset.hidden});
						} else if (asset.type == "video") {
								uploaded_videos.push({src:asset.src, thumb:asset.thumb, key:asset.key, type:"video", hidden:asset.hidden});
						}
				})
		}
	})
	...
	var path = (window.URL || window.webkitURL).createObjectURL(file);
		...
		delete data.project[0].id;
			if (data.project.length > 0) {
				...
				db.collection("projects").doc({id:1}).update(data.project[0]).then(response => {
	...
	db.collection("projects").get().then(project => {
		if (project.length > 0) {
			db.collection("assets").get().then(assets => {
				...
				$("<a />", {
					"download": "data.json",
					"href" : "data:application/json," + encodeURIComponent(JSON.stringify(exportarr))
				}).appendTo("body")
				.click(function() {
					 $(this).remove();
...
function clearProject() {
	if (window.confirm("Are you sure you want to clear this project? This action cannot be undone.")) {
